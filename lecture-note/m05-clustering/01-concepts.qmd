...
Can you identify what is missing in the description of the graph cut problem? Without this, the best cut is trivial. "<a href='https://skojaku.github.io/adv-net-sci/assets/vis/community-detection/index.html?scoreType=graphcut&numCommunities=2&randomness=1&dataFile=two-cliques.json'>Graph Cut Problem 🎮</a>"
...
Ratio cut and Normalized cut can be extended to cut into more than two communities. Let $K$ denote **the number of communities**, and let $V_1, V_2, \dots, V_K$ be the communities themselves. We use the index $c \in \{1, \dots, K\}$ to run over them, $|V_c|$ for the number of nodes in community $c$, and $|E_c|$ for the number of edges inside community $c$. Then

$$
\begin{align}
\text{Ratio cut}(V_1, V_2, \dots, V_K) &= \sum_{c=1}^K \frac{1}{|V_c|} \left(\sum_{i \in V_c} \sum_{j \notin V_{c}} A_{ij} \right) \\
\text{Normalized cut}(V_1, V_2, \dots, V_K) &= \sum_{c=1}^K \frac{1}{|E_c|} \left(\sum_{i \in V_c} \sum_{j \notin V_{c}} A_{ij} \right)
\end{align}
$$

The inner double sum counts the edges leaving community $c$, and dividing by $|V_c|$ (or $|E_c|$) penalizes the objective for making any one community tiny. Setting $K = 2$ recovers the two-community definitions above, up to the choice of writing the normalizer as a product or as a sum of reciprocals.
...
**Girvan-Newman** [@girvan2002community] works by deletion instead. Compute the *edge betweenness* of every edge (how many shortest paths run through it — see Module 6), remove the highest one, and repeat. Edges bridging communities carry the most shortest paths, so they go first, and the network splits into a dendrogram of nested communities. It is beautifully interpretable and far too slow for large networks, since betweenness must be recomputed after every removal.

::: {.callout-note title="Four different answers to 'what is a community?'"}
...
Find communities by maximizing the modularity. [Modularity maximization (two communities) 🎮](https://skojaku.github.io/adv-net-sci/assets/vis/community-detection/index.html?scoreType=modularity&numCommunities=2&randomness=1&dataFile=two-cliques.json)
...
Find communities by maximizing the modularity. [Modularity maximization (four communities) 🎮](https://skojaku.github.io/adv-net-sci/assets/vis/community-detection/index.html?scoreType=modularity&numCommunities=4&randomness=1&dataFile=two-cliques.json)
:::
...
Find communities by maximizing the modularity. [Modularity maximization (four communities) 🎮](https://skojaku.github.io/adv-net-sci/assets/vis/community-detection/index.html?scoreType=modularity&numCommunities=4&randomness=0.25&dataFile=net_karate.json)
...
Find communities by maximizing the modularity. [Modularity maximization (four communities) 🎮](https://skojaku.github.io/adv-net-sci/assets/vis/community-detection/index.html?scoreType=modularity&numCommunities=3&randomness=0.9&dataFile=two-cliques-big-clique.json)
...
Find communities by maximizing the modularity. [Modularity maximization (four communities) 🎮](../assets/vis/community-detection/index.html?scoreType=modularity&numCommunities=3&randomness=0.8&dataFile=random-net.json)
...
Tiago Peixoto made exactly this point in a much-shared post: generate an Erdős-Rényi random graph, maximize modularity, and you will be handed communities that were never there. Interested readers can follow [his thread](https://twitter.com/tiagopeixoto/status/1466352013856358400) and the [discussion that followed](https://reticular.hypotheses.org/1924).
:::

:::

The simple answer is no. Modularity is still a powerful tool for finding communities in networks. Like any other method, it has its limitations. And knowing these limitations is crucial for using it effectively. There is "no free lunch" in community detection [@peel2017ground].
...
![Adjacency Matrix of Stochastic Block Model](../figs/sbm-adjacency-matrix.svg){#fig-sbm-adjacency-matrix width="6"}
...
![Schematic of Likelihood Function (Concave)](../figs/likelihood-function.svg){#fig-likelihood-function width="6"}
...
![Adjacency Matrix of Stochastic Block Model](../figs/adjacency-matrix.svg){#fig-adjacency-matrix width="6"}
...
**Modularity** is the obvious candidate, and we have already met it. But using $Q$ to evaluate a partition that was produced by maximizing $Q$ is circular, and comparing $Q$ across different networks is meaningless (see the spurious-communities section above).
...
::: {.callout-caution title="Metadata is not ground truth"}
