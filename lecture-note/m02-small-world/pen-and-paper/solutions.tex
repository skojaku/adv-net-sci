...
Three functions, and everything else in the notebook runs off them. Everything
below except the five lines marked \verb|<--| is already on their screen; those
five are the whole of the typing.
...
Nobody writes a breadth-first search. The notebook teaches
\verb|igraph.Graph(n=..., edges=...)| and then
\verb|g.distances(source=s)| on the seven-person town first, and the blank is
those same two lines on a town it is handed. The one thing that catches people
is the \verb|[0]|: \verb|distances| takes several starting people, so it always
answers with a list of rows, and only one of them is theirs.
