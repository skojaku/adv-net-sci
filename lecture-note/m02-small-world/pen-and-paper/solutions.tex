...
them and the question may come back. Throwing the same friendships in a hat
means every pair is a friendship with the same probability $p$, decided on its
own. Each person has $n-1$ people they could befriend, so $k = p\,(n-1)$ and
$p = k/(n-1)$, which is $k/n$ once $n$ is large. Two of your friends are a pair
like any other --- their coin knows nothing about having landed on you --- so
$C_{\text{rand}} = \mans{p}$. It does not mention who your friends are, and
that is what having no structure means.
