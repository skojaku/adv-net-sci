...
- **σ（スモールワールド指数）とその破綻**。これは今 `turning-the-dial.tex` の Part 4 にあり、紙でやると対数や近似式が要って高校生には重いです。ラボの**発展課題（早く終わった人向け）**に置き、**学生自身が書いたコードが σ > 1 を出してしまう** → なぜかを式で示す → 直し方を作る、という順にします（詳細は 5 の 7 と 5b）。
...
ご要望どおり「まず見て動かす → 自分で書く → 自分のコードで動くデモ」の順にします。✍️ マークの学生が書くセルは**本編3つ＋発展1つ**（ガイドの推奨は3〜6）。本編は 6 で終わり、7 は早く終わった人向けです。
...
### 7 · 早く終わった人へ — 嘘をつく数（発展課題）

ここから先は必修ではありません。ラボの本編は 6 で終わり、速く終わった人がこれをやります。

**注意**: この節は **n = 1000 の村**で走らせます。紙の16人の村では σ も ω も差が出ません（実測: σ が 1.56 / 1.42 / 1.26 と並び、区別がつかない）。16人は手で数えるための大きさで、指標を試すための大きさではない、というのもここで学べることの1つです。

#### (a) まず結果だけ見せる（自動で走る）

学生の `distances_from` と `local_clustering` で、**近道が1本もない素のリング村**について
σ = (C/C_rand) / (L/L_rand) を計算します。n=1000 で **σ = 4.96**。

> あなたのコードは、近道がゼロの村を「強いスモールワールドだ」と判定しました。

さらに n のスライダーを動かすと、σ は 1 に近づくどころか**上がり続けます**。

#### (b) なぜか、n の式で示せるか（ヒント3段）

> **Question**: Your own code says a town with zero shortcuts is a small world, and the verdict gets *stronger* as the town grows. Show why, as a formula in `n`.

ヒントは折りたたみで3段階。答えは見ようと思えば見られますが、順に開く形にします。

- **ヒント1** — 4つの量を n の式で書いてください: `C`, `L`, `C_rand`, `L_rand`。わからなければ、上のスライダーで n を変えて測り、形を当ててください。
- **ヒント2** — その4つのうち、**n がまったく入っていない**のはどれですか。**ln でしか増えない**のはどれですか。
- **ヒント3** — 分子 `C/C_rand` と分母 `L/L_rand` をそれぞれ n の式にしてから、割ってください。
...
C/C_rand = n/8                            ← n に比例
L/L_rand = (n/8) ÷ (ln n / ln k)          ← n/ln n に比例

σ = (n/8) ÷ [ (n/8) · (ln k / ln n) ] = ln n / ln k
```

n が丸ごと約分で消え、**σ = ln n / ln k** が残ります。上限がありません。

言葉にすると: リング村はでたらめな村より「クラスタリングが n/8 倍高い」。同時に「道も n/8 倍長い」。**同じ n/8 なので割り算で完全に打ち消し合い、σ はこの村について何も測っていません。** 残ったのは打ち消し残り — 分母が `L_rand = ln n/ln k` という大きな数で割られた分の割り引きだけです。でたらめな村の道が対数でしか伸びない（＝短すぎる）せいで、リング村の道の長さのペナルティが `ln n` 分だけ軽く見積もられ、その分だけクラスタリング側が勝ちます。

**根っこの理由**: `C_rand` は `1/n` で消え、`L_rand` は `ln n` でしか増えない。この n 依存性の食い違いのせいで、**クラスタリングが n に依らず一定な網は、近道があろうがなかろうが自動的に σ > 1 になります**。リング村の `C = 1/2` はまさに一定。σ が証明していたのは「近道がある」ことではなく「クラスタリングが定数である」ことでした。

最後に確認セル（自動）: 測った σ と `ln n / ln k` を同じ軸に重ねて、**自分の式と自分のコードが一致する**のを見せます。

#### (c) 直し方を考える（✍️ 4つ目）

> **Question**: In one sentence — what did σ never look at? Then build a test that does look at it.

ヒント1つだけ:

> σ compares the town to a coin-flip town, and to nothing else. The fact that a plain ring reaches high clustering with **zero** shortcuts never enters the calculation.

✍️ セル: `def my_index(A)` を学生が書きます。

**チェックは式の形を問いません**。n=1000 の3つの町に通して、判定が分かれれば合格:

| 町 | 出てほしい判定 |
|---|---|
| 素のリング（p=0）| スモールワールドではない |
| p=0.05 で組み替えた町 | スモールワールド（3つの中で最大）|
| 完全ランダム（p=1）| スモールワールドではない |

σ はこの表の1行目で落ちます。学生の式が3つを分けられれば、どんな形でも通ります。

#### (d) 既知の答え（折りたたみ）

出版されている直し方は2つあり、**どちらも「格子」を第2の基準に足しています**。

- **ω** (Telesford et al. 2011): `ω = L_rand/L − C/C_latt`
  −1 なら格子、+1 ならランダム、0 がスモールワールド。範囲が `[−1, +1]` に収まり、n で暴走しません。
- **SWI** (Neal 2017): `SWI = (L−L_latt)/(L_rand−L_latt) × (C−C_rand)/(C_latt−C_rand)`
  0〜1。両端の基準を両方の量に使います。
...
| 町 | σ | ω | SWI |
|---|---|---|---|
| 素のリング（p=0）| **4.96** ← 合格してしまう | **−0.96** ✓ | **0.00** ✓ |
| p=0.05 | 46.8 | −0.44 | **0.82** ← 最大 |
| p=0.2 | 46.2 | 0.20 | 0.50 |
| ランダム（p=1）| 0.38 ✓ | 0.94 ✓ | 0.00 ✓ |

**同じ網に、σ は「強いスモールワールド」、ω は「ほぼ完全な格子」と言います。** しかも n を増やすと σ は ∞ へ、ω は −1 へと**逆向きに**進みます。

---

## 5b. リファレンス（基準）モデルは何がよいか

(c) の答えとして押さえてほしいのは、**基準は2つ要る**という一点です。

### 層1: 片側だけの基準は必ず壊れる

「スモールワールド」という主張は、そもそも**2つの極端の中間**だという主張です。

```
格子                    スモールワールド                  ランダム
三角形あり・近道なし  ←   三角形あり・近道あり   →   三角形なし・近道あり
```

σ はこの図の**右端としか比べていません**。だから左端（素のリング）を渡されると、「ランダムより三角形が多くて、ランダムより道が長い」としか言えず、その比が n で約分されて ln n が残ります。左端の基準を足せば、素のリングは「左端そのもの」と判定されて落ちます。これが ω と SWI がやっていることです。

**ラボで見せるのは ω** を勧めます。式が3項しかなく、−1 / 0 / +1 の意味を言葉で言えるからです。

### 層2: そのランダム基準の作り方も ER ではまずい（注記のみ）

`C_rand = ⟨k⟩/(n−1)` は Erdős–Rényi の値です。ER は全員がだいたい同じ次数になります。実際の網は次数が大きくばらついていて、次数がばらつくだけでクラスタリングは上がります。なので「ER より三角形が多い」の一部は、構造ではなく次数分布が作った見かけです。

正しい基準は **次数保存ランダム化**（configuration model、実装はダブルエッジスワップ）。同じ次数列を保ったまま辺をつなぎ替えます。格子側も同様に「同じ次数列を持つ格子化グラフ」を使うのが本筋です。

**ただしこれはラボでは実演できません。** 今回の村は全員が次数4の正則グラフなので、次数を保存してもしなくても結果が同じだからです。差が出るのは次数がばらついた実データのときだけ。なので**注記1行**にとどめ、「実際の網に使うときはここが効く」と書くだけにします。実演できないことを実演したふりをしない、という方針です。

### 正直に書いておく限界

ω と SWI は `C_latt`, `L_latt` を必要とします。今回のリング村は**それ自身が格子なので**この値がタダで手に入りますが、実データでは「同じ次数列を持つ格子」を自分で作らないといけません（リング上に並べ替えて辺の距離を最小化するスワップ）。これは自明な作業ではなく、ω が σ ほど使われていない理由でもあります。この1行はラボに入れます — 直し方にも値段がある、というのは知っておくべきことなので。
