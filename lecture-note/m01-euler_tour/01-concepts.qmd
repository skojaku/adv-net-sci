...
Try it on the map below. Put a pencil down on any landmass and trace a route that uses every bridge once. You will soon hit the same frustrating wall the citizens did: you either get stranded with a bridge left unused, or you have to cross one twice.

<!-- TODO(anim): interactive bridge tracer — click a landmass to start, click bridges to cross them, the widget refuses a second crossing of the same bridge and reports how many of the seven you managed. -->

![The seven bridges of Königsberg, drawn schematically. Four landmasses — the north bank **C**, the south bank **B**, the Kneiphof island **A** and the east island **D** — are joined by seven bridges *a*–*g*. Note that *a* and *b* both run from A to B, and *c* and *d* both run from A to C.](figs/konigsberg-map.svg){#fig-seven-bridges fig-alt="Schematic map of a river with two banks and two islands, joined by seven labelled bridges." fig-align="center" width=95%}

What makes this so difficult? No one could find a route, but more importantly, no one could *prove* it was impossible. The problem eventually reached the mathematician Leonhard Euler. His goal was not just to find an answer, but to understand the *reason* behind the answer.
...
![The same city as a graph. Every landmass has become a node and every bridge an edge; the river, the distances and the shapes are gone. The labels are the ones on @fig-seven-bridges, so you can check each edge against the map.](figs/konigsberg-graph.svg){#fig-euler-graph fig-alt="Four labelled nodes A, B, C, D joined by seven labelled edges, with the degree of each node written beside it." fig-align="center" width=80%}
...
The story of the seven bridges has an ironic epilogue. During World War II, Königsberg was heavily bombed, and two of the seven bridges — *b* and *d* in @fig-seven-bridges — did not survive.
