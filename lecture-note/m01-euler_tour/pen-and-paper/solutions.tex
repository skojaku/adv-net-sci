...
\resizebox{0.44\textwidth}{!}{\nymap{\usroad}{\tourfill \node[ordnum, pltblue] at (o11) {$\times$};}} \\[0.5em]
...
(a) Which of these cities can you drive straight \emph{through}? \arrow
\ans{2 and 4}, the ones with nothing left over. \quad (b) What do those numbers
share? \arrow \ans{They are even.} \quad (c) A left-over road has no partner.
What must this city be for the tour? \arrow \ans{Start or finish.}
...
does that drive begin and end? \arrow \ans{Yes} --- the drive existing means its
two odd cities are its ends, so it runs \ans{Ithaca} to \ans{Albany}.

For the eight roads, with US-11: can you drive every highway exactly once?
\arrow \ans{No.}

Why is there no route with eight? \arrow \ans{Four cities have a left-over
road, and a drive has only one start and one finish.}
...
\textbf{name} \\
\hline
1 2 3 2 4 & yes (2--3 twice) & yes (2) & \ans{walk} \\ \hline
1 2 3 4 2 & \ans{no} & \ans{yes (2)} & \ans{trail} \\ \hline
1 2 3 4 & \ans{no} & \ans{no} & \ans{path} \\ \hline
\end{tabular}}
\end{center}
...
\arrow \ans{1 \; 2 \; 3 \; 4 \; 2.} The lines 1--2, 2--3, 3--4, 4--2 are four
different lines; 2 is the stop that comes round twice.

{\bf Question 4(c)}: Your drive in 1(a) used no road twice, but it did come
back to a city it had already visited. Which is it --- a path, a trail, or a
walk?

\arrow \ans{A trail.} No road twice makes it a trail; coming back to a city it
...
\arrow A path of 7 roads touches \ans{8} cities --- one more city than roads,
because a path never comes back --- and the map has only \ans{4}. With nowhere
near 8 cities to visit, the drive cannot be a path.
...
Part 4 hands the student a notebook --- a computer page that runs the network
from Part 2. Worked out on paper first, then checked by the notebook.

{\bf Question 5(a)}: Write \textbf{1} in row $i$, column $j$ if a line joins
$i$ and $j$, else \textbf{0}, for the network from Part 2. \quad {\bf 5(b)}:
Add up each row.
...
Each row total counts the \ans{degree} of that node.

\end{minipage}

{\bf Question 5(c)}: A \textbf{2-step route} goes along one line, then along
one more. Count them on the network from Part 2.
...
{\bf Question 5(d)}: The notebook will square your grid. Say what you expect to
find in $A^2$.
...
\texttt{tools/build\_lab\_notebooks.py m01-euler\_tour}.}
