...
::: {.callout-important title="Degree sum formula"}
$$
\sum_{i=1}^N k_i = 2M
$$
where $M$ is the number of edges. Equivalently, the average degree is $\langle k \rangle = 2M/N$.
:::
...
A second fact follows immediately. The left side, $\sum_i k_i$, equals $2M$ and is therefore **even**. Split the sum into even-degree nodes and odd-degree nodes. The even-degree part is even. So the odd-degree part must be even too — and a sum of odd numbers is even only when there is an even *number* of them:

::: {.callout-important title="Handshaking lemma"}
In any network, the number of nodes with odd degree is even.
:::

::: {.column-margin}
The name comes from the party version: if everyone reports how many hands they shook, the number of people reporting an odd count must be even. This is also the hidden reason Euler's condition in Module 1 allows *zero or two* odd-degree nodes and never one — one is arithmetically impossible.
:::
...
#### Why it is guaranteed, not a coincidence

The four-node example is suggestive, but is the paradox *always* true? We can settle it in three lines.

Picking a random *friend* means picking a random edge endpoint. A node with degree $k$ owns $k$ of the $2M$ endpoints, so it is chosen with probability proportional to $k \cdot p(k)$, not $p(k)$. The average degree of a randomly chosen friend is therefore

$$
\langle k \rangle_{\text{friend}} = \sum_k k \cdot \frac{k\, p(k)}{\langle k \rangle} = \frac{\langle k^2 \rangle}{\langle k \rangle}.
$$

Now use the definition of variance, $\text{Var}(k) = \langle k^2 \rangle - \langle k \rangle^2$, to rewrite it:

$$
\langle k \rangle_{\text{friend}} = \frac{\langle k \rangle^2 + \text{Var}(k)}{\langle k \rangle} = \underbrace{\langle k \rangle}_{\text{your degree, on average}} + \underbrace{\frac{\text{Var}(k)}{\langle k \rangle}}_{\text{the paradox}}
$$

::: {.callout-important title="The friendship paradox, exactly"}
Your friends have, on average, $\text{Var}(k)/\langle k \rangle$ more friends than you do.

Since a variance is never negative, the gap is **never negative**: the paradox holds in *every* network. It vanishes only when $\text{Var}(k) = 0$, i.e. when everyone has exactly the same degree. And the more unequal the degrees, the larger the gap — which is why it is so dramatic in social networks and invisible in a lattice.
:::

::: {.column-margin}
The quantity $\langle k^2 \rangle / \langle k \rangle$ has a name, $\kappa$, and you have already met it: it is the degree heterogeneity that decides whether a network survives random failure in Module 3. The friendship paradox and network robustness are the same piece of mathematics wearing different clothes.
:::

::: {.callout-tip title="Check your understanding"}
Can you build a network in which the paradox is *absent* — where your friends have exactly as many friends as you? The formula tells you the answer before you draw anything: you need $\text{Var}(k) = 0$, so every node must have identical degree. A ring, a complete graph, or any regular lattice will do. Try it in the [Friendship Paradox Game 🎮](../assets/vis/friendship-paradox-game.html).
:::
...
![A histogram of a scale-free degree distribution on a linear scale. It's nearly impossible to see the structure of the tail.](../figs/linear-hist.svg){#fig-linear-hist width="6"}
...
![The same degree histogram on a log-log scale. The structure becomes much clearer, revealing a roughly linear relationship.](../figs/loglog-hist.svg){#fig-loglog-hist width="6"}
...
![The CCDF of the degree distribution on a log-log scale. ](../figs/ccdf-hist.svg){#fig-ccdf-hist width="6"}
...
![The CDF of the degree distribution on a log-log scale. ](../figs/cdf-hist.svg){#fig-cdf-hist width="4"}
...
We have described heavy-tailed degree distributions but not explained them. Why should any network develop hubs at all? A random network does not: wire $N$ nodes at random and you get a Poisson degree distribution, tightly concentrated around its mean, with no hubs worth the name.
...
::: {.callout-note title="Is 'rich get richer' the real mechanism?"}
...
## Degree correlations: who connects to whom?
...
- **Our theory's assumptions.** The Molloy-Reed derivation in the appendix assumes a node reached by an edge has $\kappa - 1$ onward edges on average. That is a *neutral*-network statement. When degrees are correlated, the branching factor changes and the predicted threshold shifts.