...
| `run_tutor.sh` | Launcher: asks continue-or-fresh, installs the marimo-pair skill, starts marimo (`--no-token`), starts the agent (pi runs with bash disabled). `--fresh` resets without asking. |
