...
| `notebook.template.py` | Pristine starter: visually blank (only hidden import/helper cells). `run_tutor.sh` copies it to `notebook.py` at each session start, archiving the previous session's notebook to `session_artifacts/`; `TUTOR_RESUME=1 ./run_tutor.sh` continues the previous notebook instead — the extension then reads the surviving `session_log.jsonl` and injects an invisible progress brief, so the tutor greets the student back and resumes at the right checkpoint (no re-teaching, no duplicate cells). `notebook.py` itself is a per-session working copy (gitignored). |
| `run_tutor.sh` | Launcher: installs the marimo-pair skill, starts marimo (`--no-token`), starts the agent (pi runs with bash disabled). |
| `.pi/extensions/say-gate.ts` | The tutor speaks only through a `say` tool. Mechanical rules (length, ≤3 sentences, one question) are checked locally with zero latency and block delivery; the LLM reviewer (never reveal what the student hasn't said) runs in the background by default — no added latency, violations become an invisible coach note before the next turn (`TUTOR_REVIEW_MODE=strict` restores blocking review; `off` disables it). Verdicts logged to `session_artifacts/reviewer_log.jsonl`. Also: plain assistant text (deliberation) is hidden, and a `think` tool provides a private reasoning channel. |
