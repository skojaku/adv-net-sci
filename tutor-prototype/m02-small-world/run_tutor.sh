...
KICKOFF="Please start the tutoring session: read lesson.yaml and begin at checkpoint cp0_welcome — unless a RESUME CONTEXT message is present, in which case greet the student back and continue where it says. Speak to the student ONLY through the say tool, use the nb_* notebook tools for all notebook work, and keep this terminal clean — the student is watching it."
if [ "$AGENT" = "pi" ]; then
