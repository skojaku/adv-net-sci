...
# describe: Six pairs of circles stacked vertically - A-B, A-C, A-D, B-C, B-D, C-D - each pair joined by a line with its shortest-path distance on top (all 1, except A-D which is 2, shown in red); the title shows the average 7/6 = about 1.17.
# --- cell: cp2_pairs_fig ---
...
for _i, (_u, _v, _d) in enumerate(_pairs):
    _y = len(_pairs) - _i
    _hl = _d > 1
    _node_color = "#e11d48" if _hl else "#60a5fa"
    _ax.plot([1, 3], [_y, _y], color="#94a3b8", lw=2, zorder=1)
    for _x, _lab in ((1, _u), (3, _v)):
        _ax.scatter([_x], [_y], s=950, c=_node_color, edgecolors="black", zorder=2)
    ...
    _ax.text(2, _y + 0.16, str(_d), ha="center", va="bottom", fontsize=15,
             fontweight="bold", color="#e11d48" if _hl else "#334155")
