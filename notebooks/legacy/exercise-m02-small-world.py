...
def _():
    ...
    edge_list_2 = [(0, 1), (1, 2), (0, 2), (0, 3)]
    src = [src for src, dst in edge_list_2]
    trg = [dst for src, dst in edge_list_2]
    weight = [1 for src, dst in edge_list_2]
    A = sparse.csr_matrix((weight, (src, trg)), shape=(4, 4))
    A = A + A.T
    A  # Make the adjacency matrix symmetric
    return (A,)
...
def _(A):
    ...
    _D = csgraph.shortest_path(A, indices=2, directed=False)
...
@app.cell
def _(A, csgraph):
    _D, predecessors = csgraph.shortest_path(A, indices=2, directed=False, return_predecessors=True)
    return
