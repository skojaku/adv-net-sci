...
@app.cell
def _(alpha, beta, g, np):
    A = g.get_adjacency_sparse()
    _c = np.linalg.inv(np.eye(g.vcount()) - alpha * A.T) @ np.ones((g.vcount(), 1)) * beta
    # %%
    _c
    return (A,)
...
def _(A, g):
    import numpy as np
    alpha, beta = (0.1, 0.05)
    _n_nodes = g.vcount()  # Hyperparameters
    _c = np.random.rand(_n_nodes, 1)  # number of nodes
    for _ in range(100):  # column random vector
        _c_next = beta * np.ones((_n_nodes, 1)) + alpha * A * _c
        if np.linalg.norm(_c_next - _c) < 1e-06:
            break
        _c = _c_next
    print(_c)
    return alpha, beta, np
...
def _(A, g, np):
    ...
    for _ in range(100):
        _c_next = beta_1 * np.ones((_n_nodes, 1)) / _n_nodes + (1 - beta_1) * P.T @ _c
        if np.linalg.norm(_c_next - _c) < 1e-06:
            break
        _c = _c_next
    print(_c)
