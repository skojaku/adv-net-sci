...
def _(g):
    import numpy as np
    _node_idx = np.random.choice(g.vs.indices)
    g.delete_vertices(_node_idx)
    print('Node removed:', _node_idx)
...
def _(g_original, np):
    ...
    for _i in range(_n_nodes - 1):
        _degree = g_1.degree()
        _node_idx = np.random.choice(g_1.vs.indices)
        g_1.delete_vertices(_node_idx)
        _connectivity = np.max(g_1.connected_components().sizes()) / g_original.vcount()
...
@app.cell
def _(g_original, np, pd, plt, sns):
    _n_nodes = g_original.vcount()
    g_2 = g_original.copy()
    _results = []
    for _i in range(_n_nodes - 1):
        _degree = g_2.degree()
        _node_idx = g_2.vs.indices[np.argmax(_degree)]
        g_2.delete_vertices(_node_idx)
        _connectivity = np.max(g_2.connected_components().sizes()) / g_original.vcount()
