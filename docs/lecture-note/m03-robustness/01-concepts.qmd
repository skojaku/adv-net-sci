...
Want to see both algorithms in action side-by-side? Check out our [**interactive Marimo notebook**](./mst-algorithms-comparison.py) that demonstrates:

- Step-by-step execution of both Kruskal's and Prim's algorithms
- Visual comparison on different graph types (simple, grid, random, power grid)
- Interactive controls to explore how each algorithm constructs the MST
- Detailed step-by-step tables showing algorithmic decisions

Run it locally with: `marimo edit mst-algorithms-comparison.py`
:::
