...
### The Fiedler vector and what the eigenvalues mean
...
- **BFS-like** (high $q$): Explore immediate neighborhoods → captures **structural roles** (structural equivalence)
- **DFS-like** (low $q$): Explore deep paths → captures **community structure** (homophily)
...
The bridge is a result of Levy and Goldberg, extended to graphs by Qiu et al. [@qiu2018network]: **skip-gram with negative sampling is implicitly factorizing a matrix.** Specifically, the optimum of the word2vec objective is reached when the dot product of two embeddings equals the shifted pointwise mutual information of the corresponding pair:

$$
{\bf u}_i^\top {\bf v}_j = \log \frac{P(i,j)}{P(i)P(j)} - \log b
$$

where $P(i,j)$ is the probability that $i$ and $j$ co-occur in a window, $P(i)$ and $P(j)$ are their marginals, and $b$ is the number of negative samples. So DeepWalk and node2vec are performing a matrix factorization after all — just of a walk-derived PMI matrix rather than of $\mathbf{A}$ or $\mathbf{L}$.

That does not make the two families interchangeable in practice:

| | Spectral | Neural |
|---|---|---|
| **Solution** | Closed form, globally optimal, deterministic | Stochastic, local optimum, different every run |
| **What it captures** | The proximity written into the matrix you chose | Higher-order proximity, set by walk length and window |
| **Scale** | Eigendecomposition is costly on very large networks | Trains by sampling; scales to millions of nodes |
| **Tuning** | Essentially just $d$ | $d$, walk length, window, $p$, $q$, negative samples, epochs |
| **Interpretability** | Eigenvalues say how much structure each dimension carries | Little direct interpretation |
...
- **Visualization.** Project to 2D and look. **t-SNE** and **UMAP** are the standard tools, and both preserve *neighborhoods*, not distances. Do not read meaning into the size of clusters or the gaps between them; those are artifacts of the projection.
...
::: {.callout-warning title="A practical consequence"}
You cannot compare individual coordinates across two runs, and you cannot compare embeddings of two networks trained separately. Only *relative* quantities within a single embedding — distances, angles, rankings — carry meaning. This trips people up constantly when they try to track how a node "moved" between two snapshots of an evolving network.
:::
