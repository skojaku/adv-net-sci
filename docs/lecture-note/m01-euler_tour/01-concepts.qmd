...
## Walk, Path, and Trail: Your Adventure Through Networks

Before we dive deeper into Euler's mathematical framework, let's establish the vocabulary that network scientists use to describe movement through graphs. Think of these as different ways to travel through a network, each with its own rules and constraints.

### Walk: The Most Flexible Journey
...
::: {.column-margin}
**Example**: In a social network, a walk might represent how gossip spreads—the same person can hear and retell the same piece of information multiple times, and the same social connection can be used repeatedly.
:::

**Formal definition**: A walk of length $k$ is a sequence of vertices $v_0, v_1, v_2, \ldots, v_k$ where each consecutive pair $(v_i, v_{i+1})$ is connected by an edge.

### Path: The Efficient Route

A **path** is a more restrictive type of walk where efficiency matters:

- Each vertex can be visited **at most once**
- Edges can still be repeated (though this rarely happens in simple paths)
- No backtracking to previously visited vertices

::: {.column-margin}
**Example**: In a computer network, a path represents the most efficient route for data packets—you don't want to revisit the same router unnecessarily, as this wastes time and resources.
:::

**Formal definition**: A path is a walk where all vertices are distinct.

### Trail: The Scenic Route

A **trail** offers a different kind of constraint, perfect for certain applications:

- Vertices can be visited multiple times
- Each edge can be traversed **at most once**
- Think of it as "never taking the same road twice"

::: {.column-margin}
**Example**: A mail carrier's route through a neighborhood—they might return to the same intersection multiple times but should traverse each street exactly once for efficiency.
:::

**Formal definition**: A trail is a walk where all edges are distinct.
