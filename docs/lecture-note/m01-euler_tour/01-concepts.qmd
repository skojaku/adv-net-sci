...
**Formal definition**: A circuit is a trail where the first and last vertices are the same.
...
**Formal definition**: A cycle is a path where the first and last vertices are the same, and no other vertex is repeated.

### Key Distinctions

The difference between cycles and circuits mirrors the difference between paths and trails:

- **Circuit**: Closed trail (distinct edges, vertices can repeat)
- **Cycle**: Closed path (distinct vertices except start/end, edges automatically distinct)

### Special Cases in Network Analysis

**Simple Cycle**: The most basic cycle with exactly 3 or more vertices. For example, a triangle (3-cycle) is the smallest possible cycle in a simple graph.

**Euler Circuit**: A circuit that traverses every edge in the graph exactly once—this was what the Königsberg citizens were actually seeking! An Euler circuit exists if and only if:
- The graph is connected, AND
- Every vertex has even degree

**Hamiltonian Cycle**: A cycle that visits every vertex in the graph exactly once. Unlike Euler's focus on edges, Hamiltonian cycles focus on vertices.

::: {.callout-important title="Euler vs. Hamiltonian"}
- **Euler Circuit**: Uses every **edge** exactly once (may revisit vertices)
- **Hamiltonian Cycle**: Visits every **vertex** exactly once (uses subset of edges)

These are fundamentally different problems! Euler's theorem gives us clear conditions for Euler circuits, but determining Hamiltonian cycles is much more complex (NP-complete).
:::

### Real-World Applications

**Circuits** are crucial for:
- **Mail delivery routes**: Cover every street (edge) exactly once
- **Snow plow routes**: Clear every road without redundancy  
- **Circuit board design**: Electrical paths that return to source

**Cycles** are essential for:
- **Delivery optimization**: Visit every customer (vertex) exactly once
- **DNA sequencing**: Reconstruct circular genomes
- **Chemical ring detection**: Identify molecular cycles

The Königsberg bridge problem was fundamentally about finding an **Euler circuit**—a way to traverse every bridge (edge) exactly once while returning home. Understanding this distinction between edge-focused (Euler) and vertex-focused (Hamiltonian) problems is crucial for applying graph theory to real-world challenges.
...
These concepts form a natural hierarchy of restrictions:

```
Walk (most general)
├── Trail (distinct edges)
│   └── Euler Trail (all edges exactly once)
└── Path (distinct vertices)
    └── Hamiltonian Path (all vertices exactly once)
```

::: {.callout-tip title="Memory Aid"}
**Walk**: "I can **walk** anywhere, anytime"
**Trail**: "I leave a **trail** behind me (no repeated edges)"
**Path**: "I follow a **path** without circling back (no repeated vertices)"
:::

Understanding these distinctions is crucial because different network problems require different types of movement. Euler's bridge problem required a trail (specifically, an Euler trail), not a path. This distinction—focusing on edges rather than vertices—was key to his solution.

In the next sections, we'll see how these concepts apply to modern network analysis and why the difference between paths, trails, and walks matters in applications from GPS routing to circuit design.
