...
def is_walk(sequence, adjacency_matrix):
    ...
    # Alternative: for loop version (slower but more explicit)
...
```{.column-margin}
**Avoiding loops**: For loops in Python is the notorious source of computational bottlenecks.
...
def is_path(sequence, adjacency_matrix, allow_cycle=False):
    ...
    # Check node uniqueness using NumPy
