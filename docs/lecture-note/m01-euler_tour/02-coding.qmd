...
def is_walk(sequence, adjacency_matrix):
    ...
    for i in range(len(sequence) - 1):
        ...
        # Check if there's an edge between current and next node
...
def is_trail(sequence, adjacency_matrix):
    ...
    # Track used edges (for undirected graphs, (i,j) same as (j,i))
    used_edges = set()
...
def classify_sequence(sequence, adjacency_matrix):
    ...
    # Check node and edge uniqueness
    nodes_unique = len(sequence) == len(set(sequence))
    
    # For closed sequences, check if all intermediate nodes are unique
    if is_closed:
        intermediate_nodes = sequence[1:-1]
        intermediate_unique = len(intermediate_nodes) == len(set(intermediate_nodes))
    else:
...
def find_all_paths(adjacency_matrix, start, end, max_length=None):
    ...
    def dfs(current_path, visited):
        ...
        for neighbor in range(len(adjacency_matrix)):
            if (adjacency_matrix[current_node][neighbor] == 1 and 
