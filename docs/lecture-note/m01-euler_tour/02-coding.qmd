---
title: Coding - From Visual to Computational
filters:
  - marimo-team/marimo
  - tikz
tikz:
...
Often we need to convert between these representations. Here's how:

```{python}
# Convert edge list to adjacency matrix
def edge_list_to_matrix(edges, num_nodes):
    matrix = np.zeros((num_nodes, num_nodes))
    for i, j in edges:
        matrix[i][j] = 1
        matrix[j][i] = 1  # For undirected networks
    return matrix

# Convert edge list to adjacency list
def edge_list_to_adjacency_list(edges, num_nodes):
    adj_list = {i: [] for i in range(num_nodes)}
    for i, j in edges:
        adj_list[i].append(j)
        adj_list[j].append(i)  # For undirected networks
    return adj_list

# Test conversions
converted_matrix = edge_list_to_matrix(edges, 5)
converted_adj_list = edge_list_to_adjacency_list(edges, 5)

print("Converted matrix matches original:", np.array_equal(matrix, converted_matrix))
print("Converted adjacency list matches original:", neighbors == converted_adj_list)
```
...
![Labeled Königsberg graph](https://github.com/skojaku/adv-net-sci/blob/gh-pages/_images/labeled-koningsberg.jpg?raw=true)

Labeled Königsberg graph
