...
def is_trail(sequence, adjacency_matrix):
    ...
    # Clever trick: Use complex numbers to represent edges!
    ...
    # Check uniqueness directly with NumPy - much cleaner!
...
**Clever trick**: We use complex numbers to represent edges! Each edge becomes `smaller_node + 1j * larger_node`. For example, edge (1,2) becomes `1+2j`, and edge (2,1) also becomes `1+2j` (normalized). This elegant approach:

- **Eliminates tuple conversion** - no need for `column_stack` or list comprehensions
- **Works directly with NumPy** - `np.unique()` handles complex numbers natively
- **Guarantees normalization** - undirected edges (1,2) and (2,1) become identical
- **Extremely efficient** - pure NumPy operations throughout
