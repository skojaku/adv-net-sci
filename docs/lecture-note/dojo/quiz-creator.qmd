...
```python {.marimo}
# Display results when run button is clicked
if run_button.value > 0 and question_input.value and question_input.value.strip():
...
"""
            ...
            mo.vstack(callouts)
elif run_button.value > 0 and not question_input.value.strip():
    mo.md("⚠️ **Warning**: Please enter a question and answer before clicking Test Questions.")
else:
    mo.md("**Ready to test questions.** Enter your question and answer above, then click 'Test Questions'.")
```
