...
mo.vstack([question_input, submit_button])
```
...
def parse_question_and_answer(raw_input: str) -> Dict[str, Any]:
    """Parse questions and answers from student input using LLM."""
    messages = [
...
# Processing status holder
processing_status = {"processing": False, "results": None}
```

```python {.marimo}
# Process questions when button is clicked
if submit_button.value and question_input.value.strip():
...
**Student Wins**: {student_wins}/{len(evaluated_results)} ({student_wins/len(evaluated_results)*100:.1f}% if evaluated_results else 0:.1f}%)"""
...
elif submit_button.value and not question_input.value.strip():
