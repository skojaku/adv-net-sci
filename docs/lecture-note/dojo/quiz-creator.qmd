...
api_key_holder = mo.ui.text(
    ...
    placeholder="API key",
    value="sk-7b747530179c4cc992159b7aaec18155",
)
...
def get_system_callout():
    if run_button.value > 1:  # Button has been clicked
        if not api_key_holder.value.strip():
            return mo.md("❌ Please enter an API key first.")
        ...
        return mo.md("Ready to test questions. Enter your question and answer above, then click 'Test Questions'.")
...
3. MODULE MISMATCH: Question content not covered in the selected module (BE VERY STRICT - if a question is about Module 1 content but Module 2 is selected, FAIL it)
...
def validate_question(question: str, answer: str, selected_module: str = None, module_context: str = None) -> Dict[str, Any]:
    ...
    # Enhanced user template that includes module context
    if selected_module and module_context:
        module_name = module_options.get(selected_module, selected_module)
        user_template = f"""Validate this quiz question and answer for the selected module:
...
REASON: [Brief explanation of decision]"""
        ...
        user_template = PROMPTS["validate_question"]["user_template"].format(question=question, answer=answer)
