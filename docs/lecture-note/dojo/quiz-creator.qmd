...
def handle_test_click(value):
    """Handle the test button click"""
    if not question_input.value or not answer_input.value or not api_key_holder.value:
        return "❌ Please fill in all required fields"

    return test_question_func(
...
```python {.marimo}
# Display the button and show its result
test_button

# Show the result if button has been clicked
if test_button.value:
    mo.md(test_button.value)
else:
    # Show instructions when no result yet
    mo.md(f"""## 📝 Test Your Question
...
- The system works best from on-campus networks""")
    ...
    # Fields are filled but button hasn't been clicked yet
    mo.md("""## ✅ Ready to Test!

**All fields are filled. Click the "🧪 Test Question" button above to start the testing process.**

The system will:
1. **Validate** your question for quality and appropriateness
2. **Test** how the AI answers your question using module content
3. **Evaluate** whether the AI got it right or wrong
4. **Provide feedback** on how to improve your question

**⏱️ Expected time:** 30-60 seconds (watch console for progress)""")
```
