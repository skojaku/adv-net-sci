...
try:
    ...
    # For environments where tomllib is not available (like Pyodide)
    # We'll handle TOML parsing manually as fallback
    tomllib = None
...
```python {.marimo}
# Question and answer input form
form = mo.ui.text_area(
...
answer = "All nodes must have even degree.""",
    ...
    full_width = True,
    rows=12
).form(submit_button_label="Test Question")
...
def parse_question_and_answer(raw_input: str) -> Dict[str, Any]:
    ...
    if "[[questions]]" in raw_input:
        if tomllib is not None:
            try:
                # Use proper TOML parsing when available
                toml_data = tomllib.loads(raw_input)
                questions = []

                # Extract questions from TOML data
                if "questions" in toml_data:
                    for q_data in toml_data["questions"]:
                ...
                if hasattr(e, '__name__') and 'TOMLDecodeError' in str(type(e)):
                    return {
                        "success": False,
                        "error": f"Invalid TOML format: {str(e)}",
                        "questions": []
                    }
                # Fall back to manual parsing if TOML parsing fails for other reasons
                pass
        
        # Manual TOML-like parsing fallback (for environments without tomllib)
        try:
            questions = []
            ...
            # Fall back to LLM parsing if manual TOML parsing also fails
            pass
...
```python {.marimo}
# Process questions when form is submitted
if form.value:
    if not api_key_holder.value.strip():
        mo.md("❌ **Error**: Please enter an API key first.")
    elif not form.value.strip():
        mo.md("⚠️ **Warning**: Please enter a question and answer before submitting.")
    else:
        # Process the questions
        results = process_questions(form.value)

        if not results["success"]:
            mo.md(f"❌ **Error**: {results['error']}")
        else:
            # Display results
            callouts = []
            ...
            success_rate = (student_wins/len(evaluated_results)*100) if evaluated_results else 0
...
**Student Wins**: {student_wins}/{len(evaluated_results)} ({success_rate:.1f}%)"""
...
"""
    ...
    mo.md("**Ready to test questions.** Enter your question and answer above, then click 'Test Question'.")
```
