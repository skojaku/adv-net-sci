...
```python {.marimo}
# System message box - shows errors, warnings, and status messages
def get_system_message():
    if run_button.value > 1:  # Button has been clicked
        if not api_key_holder.value.strip():
            return mo.md("❌ **Error**: Please enter an API key first.")
        elif not question_input.value.strip():
            return mo.md("⚠️ **Warning**: Please enter a question and answer before clicking Test Questions.")
        else:
            # Try to process and show any errors
            try:
                results = process_questions(question_input.value)
                if not results["success"]:
                    return mo.md(f"❌ **Error**: {results['error']}")
                else:
                    return mo.md("✅ **Success**: Questions processed successfully!")
...
# Display the system message box
get_system_message()
```
