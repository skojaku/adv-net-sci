...
Throughout this course, we'll primarily use `igraph`, a mature and robust library originally developed for R and later ported to Python. While `networkx` enjoys popularity for its accessibility, `igraph` offers several compelling advantages for serious network analysis: it provides more reliable algorithm implementations (avoiding persistent bugs found in some `networkx` functions like LFR benchmark and weighted degree assortativity), and its optimized C core delivers the performance needed for large-scale network computations.

For advanced users comfortable with `scipy`, the `csgraph` submodule provides an excellent alternative that leverages one of Python's most battle-tested and optimized libraries. Functions like `csgraph.shortest_path` and `csgraph.connected_components` offer high-performance implementations backed by `scipy`'s proven track record. You can explore more about using `scipy` for network analysis in the [Appendix](appendix.qmd).
