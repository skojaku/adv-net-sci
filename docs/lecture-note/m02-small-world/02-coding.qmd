...
The local clustering coefficient measures the fraction of possible triangles that actually exist around each node. For a node with degree k, there are k(k-1)/2 possible edges between its neighbors. The `transitivity_local_undirected()` function computes this for each node, returning a list of float values (one per node). The formula is: (actual edges between neighbors) / (k(k-1)/2), and it returns `NaN` for nodes with degree < 2.
...
The average local clustering coefficient is simply the mean of all local clustering coefficients across nodes. The `transitivity_avglocal_undirected()` function computes this directly, returning a single float value. It automatically ignores NaN values from low-degree nodes when calculating the mean.
...
The global clustering coefficient (also called transitivity) is defined as 3 × (number of triangles) / (number of connected triples), where a "connected triple" is a path of length 2. The `transitivity_undirected()` function computes this single float value, giving more weight to high-degree nodes since they contribute more triples to the denominator.
...
We can use igraph's built-in network generators to create different network types for comparison: `Graph.Full()` creates complete graphs, `Graph.Erdos_Renyi()` creates random graphs, and `Graph.Lattice()` creates regular lattices. This allows us to compare clustering patterns across different topologies.
...
The `Graph.Watts_Strogatz()` function creates small-world networks using the Watts-Strogatz model, which starts with a ring lattice and randomly rewires edges with probability p. We can also use `average_path_length()` to compute the mean shortest path length across all node pairs.
