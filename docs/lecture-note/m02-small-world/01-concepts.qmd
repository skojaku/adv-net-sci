...
Average path length is the average number of edges in the shortest path between any two nodes. Two nodes can have multiple paths between them, and the **distance** between the two nodes is defined as the length of the shortest path. The length of a path is the number of edges in the path.

```{dot}
//| fig-cap: "Example network for calculating average path length"
graph G {
  layout=fdp
  A -- B;
  A -- C;
...
Let's calculate the average path length for this network:

- **Distance from A**: A to B (1), A to C (1), A to D (2), A to E (2), A to F (3)
- **Distance from B**: B to A (1), B to C (1), B to D (1), B to E (2), B to F (2)  
- **Distance from C**: C to A (1), C to B (1), C to D (1), C to E (1), C to F (2)
- **Distance from D**: D to A (2), D to B (1), D to C (1), D to E (1), D to F (1)
- **Distance from E**: E to A (2), E to B (2), E to C (1), E to D (1), E to F (1)
- **Distance from F**: F to A (3), F to B (2), F to C (2), F to D (1), F to E (1)

The average path length is: (1+1+2+2+3 + 1+1+1+2+2 + 1+1+1+1+2 + 2+1+1+1+1 + 2+2+1+1+1 + 3+2+2+1+1) ÷ 30 = 45 ÷ 30 = **1.5**

This means that, on average, any two nodes in this network are separated by 1.5 edges.
