...
There are two ways to measure clustering: **local clustering** and **global clustering**. The key difference is the focus of the measurement:

- **Local clustering** focuses on the clustering of a node's immediate neighbors.
...
where a **connected triplet** is a set of three nodes connected by at least two edges (forming either a triangle or a "wedge" shape).

```{dot}
//| fig-width: 5
//| fig-height: 2.5
//| fig-cap: "Connected triplets: closed (triangle) vs open (wedge)"
//| fig-align: center
graph G {
  layout=neato;
  fontsize=16;
  ...
  subgraph cluster_1 {
    ...
    style=filled;
    color=lightgrey;
    A2 [pos="1.5,0!" shape=circle];
    B2 [pos="0.5,-1!" shape=circle];
    C2 [pos="2.5,-1!" shape=circle];
    A2 -- B2;
    B2 -- C2;
  }
}
...
A **closed triplet** forms a complete triangle, while an **open triplet** forms a wedge shape (missing one edge to complete the triangle).

```{dot}
//| fig-width: 3
//| fig-height: 2.5
//| fig-cap: "Why one triangle counts as three triplets"
//| fig-align: center
graph G {
  layout=neato
  A [pos="0,1.5!", style=filled, fillcolor="#ffcccc"]
  B [pos="-1,0!", style=filled, fillcolor="#ccffcc"]
  C [pos="1,0!", style=filled, fillcolor="#ccccff"]

  A -- B [color=red, penwidth=2]
  B -- C [color=green, penwidth=2]
  C -- A [color=blue, penwidth=2]
}
```

This triangle contains **three connected triplets**:
- Triplet 1: A-B-C (centered at B, red-green path)
- Triplet 2: B-C-A (centered at C, green-blue path)
- Triplet 3: C-A-B (centered at A, blue-red path)

Each triplet has a different "center" node that connects to the other two.

Alternatively, we can express this using the adjacency matrix $A$:

$$
C = \frac{\text{tr}(A^3)}{\sum_i k_i(k_i-1)}
$$

where $\text{tr}(A^3)$ is the trace of $A^3$ (sum of diagonal elements, which counts triangles), and $k_i$ is the degree of node $i$.

::: {.callout-note collapse="true" title="Note: Why multiply by 3 in the numerator?"}
Each triangle contributes to exactly 3 connected triplets (one for each vertex of the triangle). Therefore, we multiply the number of triangles by 3 to get the number of "closed" triplets. The denominator counts all possible triplets (both closed and open), so the ratio gives us the fraction of triplets that form complete triangles.
:::

The global clustering coefficient ranges from 0 (no triangles) to 1 (every connected triplet forms a triangle).
