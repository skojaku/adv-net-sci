...
- A **complete bipartite network** has short paths but zero clustering, giving $s_{\text{naive}} = 0$
- A **ring lattice** has high clustering but long paths in large networks
...
$$
\sigma = \frac{C/C_{\text{random}}}{L/L_{\text{random}}} = \frac{C \cdot L_{\text{random}}}{L \cdot C_{\text{random}}}
$$

where:
- $C$ is the clustering coefficient of the network
- $L$ is the average path length of the network
- $C_{\text{random}}$ is the clustering coefficient of an equivalent random network
- $L_{\text{random}}$ is the average path length of an equivalent random network

**Interpretation:**
- $\sigma \gg 1$: Strong small-world property (high clustering relative to random, short paths relative to random)
- $\sigma \approx 1$: Similar to random networks
- $\sigma \ll 1$: Anti-small-world behavior

**Why this normalization works:**
- Random networks typically have low clustering ($C_{\text{random}} \ll C$) but short paths ($L_{\text{random}} \approx L$)
- Small-world networks maintain high clustering while achieving short paths similar to random networks
- This gives $\sigma = \frac{C \gg C_{\text{random}}}{L \approx L_{\text{random}}} \gg 1$

The "equivalent random network" typically refers to an Erdős–Rényi random graph with the same number of nodes and edges (or same average degree) as the original network.
