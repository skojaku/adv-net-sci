...
### Step 4: Create Train/Validation/Test Masks

We'll split the labeled nodes into three sets: training (for learning), validation (for hyperparameter tuning and early stopping), and test (for final evaluation).

::: {.column-margin}
**Why Validation?** The validation set helps us monitor overfitting during training and tune hyperparameters without touching the test set, which should only be used for final evaluation.
:::
...
# Split nodes per class: 2 train, 1 validation, rest test
for label in [0, 1]:
    label_indices = np.where(membership == label)[0]
    np.random.shuffle(label_indices)

    # 2 nodes for training, 1 for validation per class
    train_mask[label_indices[:2]] = True
    val_mask[label_indices[2:3]] = True
    test_mask[label_indices[3:]] = True
...
print(f"Validation node indices: {torch.where(val_mask)[0].numpy()}")
```
...
::: {.column-margin}
**GCNConv**: PyTorch Geometric's implementation of Graph Convolutional Network layer from [Kipf & Welling (2017)](https://arxiv.org/abs/1609.02907). It performs neighborhood aggregation with symmetric normalization: $\mathbf{X}^{(l+1)} = \sigma(\mathbf{\hat{D}}^{-1/2}\mathbf{\hat{A}}\mathbf{\hat{D}}^{-1/2}\mathbf{X}^{(l)}\mathbf{W}^{(l)})$.
...
::: {.column-margin}
**ReLU** (Rectified Linear Unit): Activation function $\text{ReLU}(x) = \max(0, x)$ that introduces non-linearity. It outputs the input if positive, otherwise zero. This allows the network to learn complex patterns.
...
Now let's train the GNN model using gradient descent, monitoring performance on the validation set.
...
::: {.column-margin}
**Monitoring Validation**: By tracking validation accuracy, we can detect overfitting (when training accuracy increases but validation accuracy decreases) and implement early stopping if needed.
:::

### Step 7: Visualize Training Progress

Let's visualize how the model learned over time, including validation performance.
...
# Plot loss
axes[0].plot(losses, color='#2E86AB')
axes[0].axvline(best_epoch, color='red', linestyle='--', alpha=0.5, label=f'Best Val (epoch {best_epoch+1})')
...
# Plot accuracy
axes[1].plot(train_accs, label='Train', color='#2E86AB', linewidth=2)
axes[1].plot(val_accs, label='Validation', color='#A23B72', linewidth=2)
axes[1].plot(test_accs, label='Test', color='#F18F01', linewidth=2)
axes[1].axvline(best_epoch, color='red', linestyle='--', alpha=0.5)
axes[1].set_xlabel('Epoch')
axes[1].set_ylabel('Accuracy')
axes[1].set_title('Accuracy: Train / Validation / Test')
axes[1].legend()
