...
graph G {
    layout=neato;
    node [shape=circle, style=filled, fontsize=14, width=0.9];
    edge [len=1.5];
    ...
    subgraph cluster_kplex {
        ...
        d -- e; d -- f;
        e -- f;
        // Each node connected to 4 out of 5 others (misses exactly 1)
    }
    ...
    subgraph cluster_not_kplex {
        ...
        g [pos="5.5,2.5!", fillcolor="#ff9999"];
        h [pos="6.5,3!", fillcolor="#ff9999"];
        i [pos="7,2!", fillcolor="#ff9999"];
        j [pos="6.5,1!", fillcolor="#ff9999"];
        k [pos="5.5,1.5!", fillcolor="#ff9999"];
        l [pos="6,2!", fillcolor="#ff9999"];
        ...
        j -- k; j -- l;
        k -- l;
        // Node g misses 3 connections (to i, j, l), violating k=2 constraint
    }
}
```
...
graph G {
    ...
    // ρ-dense: qualifies (has 12/15 = 80% > 75% threshold)
    subgraph cluster_dense {
        label="✅ ρ-dense (ρ=0.75)\n12/15 edges = 80% density";
        style=filled;
        fillcolor="#e8f5e8";
        ...
        e -- f;
        b -- d; b -- e;
        // Missing: a -- c, a -- d, c -- e (has 12 out of 15 possible edges = 80%)
    }

    // Not ρ-dense: fails (has 6/15 = 40% < 75% threshold)
    subgraph cluster_not_dense {
        label="❌ NOT ρ-dense (ρ=0.75)\n6/15 edges = 40% density";
        style=filled;
        fillcolor="#ffe8e8";
        ...
        j -- k;
        k -- l;
        // Missing 9 edges: has only 6 out of 15 possible edges = 40%
    }
}
```
...
graph G {
    ...
    subgraph cluster_not_truss {
        ...
        j -- k;
        k -- l; l -- g;
        // Some edges (like h-l) participate in <2 triangles
    }
}
```
