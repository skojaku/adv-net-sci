...
- **Real insight:** Connectivity is a prerequisite for Network Science
...
::: {.callout-note title = "Challenge:"}
**Given any network, can we write a function to detect if it has an Euler path?**
:::
...
**Which would be fastest for finding a node's degree?**
:::
...
::: {.incremental}
- Sum of row 1: `matrix[1].sum()` = **3** ✅
:::

## Computing Degrees: Speed Comparison ⚡

::: {.callout-note title = "Performance Challenge:"}
**Which method is fastest for computing ALL node degrees?**
:::
...
**Winner:** Matrix operations leverage optimized linear algebra!

## Implementing Euler's Theorem 🧮

**Step 1:** Count odd-degree nodes
...
::: {.callout-note title = "Euler's Complete Theorem:"}
**An Euler path exists if and only if:**
...
**Problem:** How do we check if a network is connected?

*Think about it... what algorithm could help?*

## Connected Components with DFS 🔍

**Algorithm:** Start from any node, explore as far as possible
...
```python
# Test with Königsberg bridges
konigsberg = np.array([
    [0, 1, 1, 1],  # All nodes have degree 3
    [1, 0, 1, 1],
    [1, 1, 0, 1], 
    [1, 1, 1, 0]
])

print("Königsberg has Euler path:", has_euler_path_complete(konigsberg))
```

::: {.callout-note title = "Question:"}
**What should this print? Why?**
:::

::: {.incremental}
- **Answer:** `False` - Four nodes with odd degree!
:::

## Summary: From Bridges to Code 🌉→💻

**What we learned:**

::: {.incremental}
1. **Multiple representations** → Choose based on your task
2. **Vectorized operations** → NumPy beats loops
3. **Complete algorithms** → Don't forget connectivity!
4. **270+ years later** → Euler's insight still guides our code
:::

**Next:** Let's explore more network properties!
