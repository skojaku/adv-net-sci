...
**Remarkable finding**: This optimization naturally emerges in **hyperbolic space**!
...
1. **Sequential node addition**: At each time $t = 1, 2, 3, \dots$, add a new node $t$
2. **Each node gets coordinates**:
   - **Similarity** $\theta_t$: Random position on a circle
   - **Popularity** $r_t = \ln t$: Based on birth time (older = more popular)
3. **New node connects**: Choose $m$ existing nodes $s < t$

::: {.callout-tip title = "The Connection Rule:"}
Node $t$ connects to the $m$ nodes that minimize: $s \cdot \theta_{st}$
:::
...
**Similarity Coordinate ($\theta_t$)**:
- Position on a circle
- Angular distance $\theta_{st}$ = similarity measure
- Small $\theta_{st}$ → nodes are similar

**Popularity Coordinate ($r_t$)**:
- Based on birth time: $r_t = \ln t$
- Small $t$ → small $r_t$ → older node
- Older nodes = more popular
...
*Radial position = birth time, Angular position = similarity*
...
::: {.callout-note title = "Question:"}
When will this product $s \cdot \theta_{st}$ be small?

Think about it for 30 seconds...
:::

### When is $s \cdot \theta_{st}$ small?

::: {.incremental}
- **Small if $s$ is small**: Node is old (popular)
  - Connect to popular nodes!
- **Small if $\theta_{st}$ is small**: Node is similar
  - Connect to similar nodes!
- **Key insight**: Natural trade-off emerges
  - Very popular node? Connect even if not similar
  - Very similar node? Connect even if not popular
:::

::: {.callout-tip title = "Why This Works:"}
The product $s \cdot \theta_{st}$ elegantly balances popularity and similarity without explicit weights or parameters!
:::

### Why does this explain preferential attachment?
...
- But with crucial differences: **which** nodes connect depends on similarity
:::
...
::: {.callout-note title = "Question:"}
Look at the WordNet embedding above.

Where are abstract concepts like "Entity"? Where are concrete concepts like "Hardwood"?

*What pattern do you notice?*
:::

### Answer: Natural Hierarchy Emerges! 🌳
...
- Standard flat geometry
- Works for many networks
- Polynomial volume growth

**Limitations**:
- Struggles with hierarchies
- Inefficient for trees
- Distorts scale-free structure
...
- Curved negative-curvature space
- Exponential volume growth
- Natural for hierarchies

**Advantages**:
- Captures scale-free properties
- Preserves strong clustering
- Efficient tree embeddings
- Explains popularity × similarity
